...
object_allocator::object_allocator(size_t object_size, oa_config const& config) :
	...
	if (mConfig.m_alignment > 1) {
		mConfig.m_inter_align_size = mConfig.m_alignment -
			(object_size + mConfig.m_hblock_info.m_size)
			% mConfig.m_alignment;
		mConfig.m_left_align_size = mConfig.m_alignment - (
			mConfig.m_hblock_info.m_size + sizeof(void*)) % mConfig.m_alignment;
	}

	mStat.m_page_size = sizeof(generic_object*) + mConfig.m_left_align_size + mConfig.m_objects_per_page *
		(object_size + mConfig.m_hblock_info.m_size)
		+ mConfig.m_inter_align_size * (mConfig.m_objects_per_page - 1);
	allocate_new_page_();
}
...
void* object_allocator::allocate(char const* label) {
	...
	if (!mStat.m_free_objects || !mFreeList) {
		//Check that we have some spare pages
		if (mStat.m_pages_in_use == mConfig.m_max_pages)
			throw oa_exception(oa_exception::E_NO_PAGES, "No more free objects on page");
		else
			allocate_new_page_();
	}
...
void object_allocator::allocate_new_page_() {
	...
	catch (...) { throw oa_exception(oa_exception::E_NO_MEMORY, "Keyword new raised an Exception"); }
	...
	for (size_t i = 0; i < mConfig.m_objects_per_page; i++)
		mFreeList = set_page_ptr_at_index(temp_, i, mFreeList);

	mStat.m_free_objects = mConfig.m_objects_per_page;
}
...
const noexcept {
	return reinterpret_cast<generic_object*>(reinterpret_cast<char*>(page) + sizeof(void*) +
		mConfig.m_left_align_size +
		((mStat.m_object_size + mConfig.m_hblock_info.m_size
			+ mConfig.m_inter_align_size)
			* index + mConfig.m_hblock_info.m_size));
}
...
void object_allocator::configure_headers(generic_object* page) noexcept {
	...
	for (size_t i = 0; i < mConfig.m_objects_per_page; i++)
		memset(get_object_header(get_object_ptr(page, i)), 0, mConfig.m_hblock_info.m_size);
}
...
void inline* object_allocator::get_object_header(generic_object* object) const noexcept {
	return reinterpret_cast<char*>(object) - mConfig.m_hblock_info.m_size;
}
