...
void ImGui::ColorConvertRGBtoHSV(float r, float g, float b, float& out_h, float& out_s, float& out_v)
{
	...
	out_h = ImFabs(T + (g - b) / (6.f * chroma + 1e-20f));
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
	...
	struct Funcs
	{
		...
		static void NodeStorage(ImGuiStorage* storage_t, const char* label)
		{
			if (!ImGui::TreeNode(label, "%s: %d entries, %d bytes", label, storage_t->Data.Size, storage_t->Data.size_in_bytes()))
				return;
			for (int n = 0; n < storage_t->Data.Size; n++)
			{
				const ImGuiStorage::ImGuiStoragePair& p = storage_t->Data[n];
