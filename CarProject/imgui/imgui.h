...
#ifndef IM_ASSERT
#include <assert.h>
#define IM_ASSERT(_EXPR)            ;                               // You can override the default assert handler by editing imconfig.h
#endif
...
namespace ImGui
{
	...
	IMGUI_API void          SetStateStorage(ImGuiStorage* storage_t);                             // replace current window storage with our own (if you want to manipulate it yourself, typically clear subsection of it)
...
struct ImDrawList
{
	...
	IMGUI_API void  AddPolyline(const ImVec2* points, const int num_points, const ImU32 col, const bool closed, float thickness) noexcept;
